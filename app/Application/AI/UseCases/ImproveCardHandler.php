...
final class ImproveCardHandler
{
    ...
    public function handle(ImproveCardCommand $command): AIJob
    {
        $payload = [
            'title' => $command->title,
        ...
        $job = $this->jobs->create([
            ...
            'type' => AIJobType::CARD_IMPROVE,
            'payload' => $payload,
            'status' => JobStatus::PENDING,
        ]);

        $this->logger->info('AI card improve job created', [
            'job_id' => $job->id,
            'user_id' => $command->userId,
        ]);
