...
final class DecomposeCardFormHandler
{
    ...
    public function handle(DecomposeCardFormCommand $command): AIJob
    {
        $payload = [
            'title' => $command->title,
        ...
        $job = $this->jobs->create([
            ...
            'type' => AIJobType::CARD_DECOMPOSE,
            'payload' => $payload,
            'status' => JobStatus::PENDING,
        ]);

        $this->logger->info('AI card decompose form job created', [
            'job_id' => $job->id,
            'user_id' => $command->userId,
        ]);
