...
final class CompleteFocusSessionHandler
{
    ...
    public function handle(CompleteFocusSessionCommand $command): DomainFocusSession
    {
        return DB::transaction(function () use ($command) {
            $session = $this->sessionRepository->findOrFail($command->sessionId);

            
            $this->focusSessionDomainService->validateStatusTransition($session, 'completed');
